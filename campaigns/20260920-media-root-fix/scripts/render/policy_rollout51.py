...
try:
    ...
    if args.steps + args.settle_steps >= int(np.ceil(cfg.episode_length_s / step_dt)) - 1:
    ...
    render_integrity = {"validated_render_calls": 0, "first_frame": None, "final_frame": None}
    ...
    def checker_details():
        ...
        result = gbv.success_checker_garment_fold(obj, garment_type)
    ...
    for i in range(args.steps):
        ...
        if not success:
            s = env._get_success()
            checked_success = bool(np.asarray(s.detach().cpu() if hasattr(s, "detach") else s).ravel()[0])
    ...
    terminal_s = env._get_success()
    terminal_success = bool(np.asarray(terminal_s.detach().cpu()
                            if hasattr(terminal_s, "detach") else terminal_s).ravel()[0])
    terminal_details = checker_details()
    verdict = audit.finish(terminal_success)
    success = verdict["success"]
    final_images = render_images()
    n_rendered += 1
    keep_frame(audit.completed_steps, final_images)
    ...
    render_integrity["first_to_final_changed_pixel_fraction"] = {
        ...
        for key in camera_keys}
    result = {
        **verdict, "schema_version": 2, "status": "completed",
