"""CPU tests for v4: the scaled coverage gate, the preregistered improvement
rule and checkpoint rule, the manifest's separation guarantees, the
whole-episode anchor, and the driver's refusal to invent a second attempt.
...
# ------------------------------------------------------------ coverage gate
def test_v3_scale_coverage_would_not_pass_v4s_gate():
    """v4's gate is scaled to the 3x search: v3's own passing coverage
    (6 spread successes) must NOT satisfy it -- the thresholds tightened
    with the attempt budget, they did not loosen."""
    attempts = {f"r{i}": [_att(60, True), _att(120, True), _att(240, False), _att(360, False)] for i in range(3)}
    g = brd.coverage(attempts, GATE)
    assert not g["passed"] and "successful_branches" in g["unmet_requirements"]


def test_spread_successes_pass_and_failures_are_counted():
    attempts = {f"r{i}": [_att(r, True) for r in (60, 120, 180, 240)] + [_att(300, False)] * 4
                for i in range(5)}
    g = brd.coverage(attempts, GATE)
    assert g["passed"] and g["successful_branches"] == 20
    assert g["attempts_completed"] == 40 and g["success_rate"] == pytest.approx(0.5)
...
def test_candidate_set_and_roots_are_fixed_before_search():
    ...
    assert all(c["horizon"] == 10 for c in rec["candidates"]), "v4 is H10-only by design"
    assert len({c["seed"] for c in rec["candidates"]}) == 8
    assert rec["roots"] == [60, 120, 180, 240, 300, 360]
    assert all(0 < r < 600 and r % 10 == 0 for r in rec["roots"])


def test_search_seeds_are_fresh_and_the_final_set_is_v3s_verbatim():
    v3 = json.loads((ROOT.parent / "20260923-recovery-supervision-v3/manifest.json").read_text())
    v3_seeds = {r["seed"] for r in v3["recovery_search"]}
    assert not ({r["seed"] for r in MANIFEST["recovery_search"]} & v3_seeds)
    assert MANIFEST["final_test"] == v3["final_test"], "the unseen final set must carry over unchanged"


def test_recipe_factor_and_checkpoint_rule_are_preregistered():
    tr = MANIFEST["training"]
    assert tr["anchor_mode"] == "whole_episode" and tr["grad_accum"] == 8
    assert tr["checkpoint_every"] == 25 and tr["checkpoint_rule"] == "latest_guard_passing"
...
def test_no_second_attempt_without_a_committed_evidence_plan(tmp_path):
    ...
    assert d.attempt_plan(2) is None           # no default ladder in v4 either
    plan1 = d.attempt_plan(1)
    assert plan1["dataset"] == "recovery"
    assert plan1["checkpoint_rule"] == "latest_guard_passing", \
        "attempt 1 must inherit the manifest's preregistered checkpoint rule"


def test_whole_episode_anchor_spreads_frames_and_records_mode(tmp_path):
    trainer = _load("rollout_weighted_finetune")
    ep = tmp_path / "ep0.npz"
    np.savez(ep, images=np.zeros((200, 3, 480, 640, 3), np.uint8), state=np.zeros((200, 12), np.float32),
             action=np.zeros((200, 50, 12), np.float32), garment=np.asarray("Top_Short_0"),
             episode=np.asarray(0))
    x, s, a, rec = trainer.load_raster_subset(str(tmp_path / "ep*.npz"), 1, 12,
                                              heldout=set(), mode="whole_episode")
    assert rec[0]["mode"] == "whole_episode"
    idx = rec[0]["frames_used"]
    assert idx[0] == 0 and idx[-1] == 199 and len(idx) == 12, \
        "whole-episode anchor must span the episode, not its first frames"
    x2, _, _, rec2 = trainer.load_raster_subset(str(tmp_path / "ep*.npz"), 1, 12,
                                                heldout=set(), mode="episode_start")
    assert rec2[0]["frames_used"] == list(range(12))
...
def test_every_v3_phase_resolves_its_rows(phase, index):
    ...
    assert request["runner"]["path"].endswith("20260923-recovery-supervision-v4/scripts/render/policy_rollout51.py")
