...
def main() -> int:
    ...
    horizon, rel = example_classes(data, manifest, root, len(origin))
    ...
    return run_gate(args, manifest, root, device, data, origin, kind, prov, rng, pick,
                    images, state, action, ret_x, ret_s, ret_a, self_aligned,
                    load, predict, errors)


def example_classes(data, manifest: dict, root: Path, n: int):
    """(horizon, step - root) per example. v6's corpus merges two campaigns'
    searches, so the row-order reconstruction below cannot work for it; the
    compiled dataset has carried these provenance columns since v4 (student
    examples are -1), and they are used whenever present. The reconstruction
    stays for datasets that predate the columns."""
    if {"horizon", "root", "step_index"} <= set(data.files):
        horizon = np.asarray(data["horizon"], dtype=np.int64)
        root_col = np.asarray(data["root"], dtype=np.int64)
        rel = np.where(horizon >= 0, np.asarray(data["step_index"], dtype=np.int64) - root_col, -1)
        if len(horizon) != n:
            raise SystemExit(f"provenance columns misaligned: {len(horizon)} != {n}")
        return horizon, rel
    # Reconstructed from the per-row search records (the compiled dataset
    # predates the provenance columns; the rows were verified byte-identical
    # to their sources, in manifest order).
    origin = np.arange(n)
