...
v6 copy of v5's matched runner. The only change: the candidate is produced
by this campaign's training, so it cannot be pinned in the manifest at
freeze time. The driver writes audit/candidate_checkpoint.json (path, label,
SHA-256 of every file) when it selects the checkpoint, and every matched
task verifies the candidate against that record exactly as v5 verified its
manifest pin. The baseline stays pinned in the manifest. Nothing unpinned
can run.
...
PHASES = ("smoke", "benchmark", "final_test")
...
def checkpoint_block(root: Path, manifest: dict, which: str) -> dict:
    """The pinned record for one policy: the manifest's baseline, or the
    candidate record the driver wrote at selection."""
    if which == "baseline":
        return manifest["baseline_checkpoint"]
    record = root / "audit" / "candidate_checkpoint.json"
    if not record.is_file():
        raise SystemExit("no candidate selected yet: audit/candidate_checkpoint.json is missing")
    block = json.loads(record.read_text())
    if not {"path", "label", "sha256"} <= set(block):
        raise SystemExit("candidate record lacks path, label or sha256")
    return block
...
def main() -> int:
    ...
    with (dest / "rollout.log").open("w") as log:
        ...
        try:
            timeout_key = "smoke" if args.phase == "smoke" else "benchmark"
