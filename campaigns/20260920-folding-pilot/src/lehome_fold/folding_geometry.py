...
def bind_landmarks(obj, mesh_path):
    ...
    expected = obj.transform_points(unique, np.asarray(obj.init_pos),
                                    np.asarray(obj.init_ori), np.asarray(obj.init_scale))
    proof = verify_initial_correspondence(live, expected)
