...
ap.add_argument("--boundary_eval_trained_paths", nargs="+", default=[],
                help="multiple trained checkpoints for the snapshot-only comparison")
ap.add_argument("--boundary_eval_out", default="",
                help="JSON output for the snapshot-only baseline/trained comparison")
ap.add_argument("--boundary_execute_trained_path", default="",
...
ap.add_argument("--recovery_out", default="",
                help="v3: JSON record of every recovery-search attempt")
ap.add_argument("--recovery_examples_out", default="",
                help="v3: NPZ of observation -> executed-continuation labels from SETTLED-successful branches only")
ap.add_argument("--recovery_roots", default="50,150,250,350",
                help="v3: student-visited root steps (actions executed) at which to snapshot and branch")
ap.add_argument("--recovery_candidates", default="10:9101,10:9102,50:9103,50:9104",
                help="v3: fixed candidate continuations as horizon:torch_seed pairs, tried at every root")
ap.add_argument("--recovery_obs_every", type=int, default=10,
                help="v3: observation sampling stride along a branch")
ap.add_argument("--recovery_max_seconds", type=float, default=5400.0,
                help="v3: wall-clock budget for all branches; remaining attempts are recorded as not run")
...
args = ap.parse_args()
if bool(args.recovery_out) != bool(args.recovery_examples_out):
    ap.error("recovery search needs both --recovery_out and --recovery_examples_out")
try:
    args.recovery_roots = tuple(sorted({int(x) for x in args.recovery_roots.split(",") if x.strip()}))
    args.recovery_candidates = tuple(
        (int(h), int(s)) for h, s in (item.split(":") for item in args.recovery_candidates.split(",") if item.strip()))
except ValueError:
    ap.error("invalid --recovery_roots or --recovery_candidates")
if args.recovery_out and any(h not in (5, 10, 25, 50) for h, _ in args.recovery_candidates):
    ap.error("recovery candidate horizons must divide the 50-action chunk")
if args.recovery_out and any(r <= 0 or r >= args.steps or r % 10 for r in args.recovery_roots):
    ap.error("recovery roots must be H10 replan boundaries strictly inside the episode")
...
if args.boundary_execute_trained_path and boundary_eval_paths:
    ap.error("boundary execution cannot be combined with boundary evaluation")
...
if boundary_eval_paths:
    ...
    boundary_eval_paths = [os.path.abspath(os.path.expanduser(path))
                           for path in boundary_eval_paths]
    args.boundary_eval_trained_path = boundary_eval_paths[0]
    args.boundary_eval_trained_paths = boundary_eval_paths
...
if args.boundary_execute_trained_path:
    if not args.causal_out:
        ap.error("boundary execution requires --causal_out for exact source snapshots")
    if args.rtc_guidance or args.queue_diagnostic or args.observation_diagnostic or args.observation_execute_best:
        ap.error("boundary execution cannot use RTC, queue, or observation diagnostics")
...
try:
    ...
    def policy_batch_for(processor, images, joint):
        batch = make_observation(images, joint)
        return processor(batch) if processor else batch

    def policy_batch(images, joint):
        return policy_batch_for(pre, images, joint)
    ...
    hard_queue_handoffs = []
    causal_snapshots = {}
    recovery_enabled = bool(args.recovery_out)
    recovery_snapshots = {}
    causal_trace = {}
    executed_actions = {}
    ...
    for i in range(args.steps):
        ...
        if recovery_enabled and (i + 1) in args.recovery_roots:
            # Root = the state after i+1 student actions, i.e. exactly what the
            # student observes before its next action. A root is captured on
            # the student's own trajectory; nothing about the future is known.
            recovery_snapshots[i + 1] = _snapshot_state(i + 1, render_images())
            recovery_snapshots[i + 1]["conditions_passed"] = int(geometric["conditions_passed"])
            log(f"RECOVERY_ROOT step={i + 1} conditions={geometric['conditions_passed']}")
    ...
    if recovery_enabled:
        # Bounded, simulator-validated recovery search. From every captured
        # student-visited root, every preregistered candidate continuation is
        # EXECUTED to the end of the episode and scored by the unchanged
        # settle protocol. Every attempt is recorded, successful or not. Only
        # a continuation that demonstrably reaches settled success becomes a
        # label, and a label pairs the observation actually rendered at that
        # state with the actions actually executed from it. Nothing cached is
        # replayed after the root, and no plan is scored without execution.
        def _rec_joint():
            return np.concatenate([_as_cpu_array(env.left_arm.data.joint_pos[0]),
                                   _as_cpu_array(env.right_arm.data.joint_pos[0])]).astype(np.float32)

        def _rec_measure(points, links, geometry):
            """v7 branch telemetry, passive: the same quantities the main loop's
            BehaviorTelemetry records (gripper/jaw link-origin distance to the
            nearest particle, particle lift above the pre-policy cloth, and the
            checker's per-condition margins), from values already computed for
            this step. No render, physics step or RNG draw."""
            dist = []
            for side in ("left", "right"):
                origins = np.asarray(links[side][0])[behavior.indices[side]]
                dist.append(float(np.linalg.norm(points[None, :, :] - origins[:, None, :], axis=2).min()))
            lift = float((points[:, 2] - behavior.initial_z).max())
            margins = [float(geometry["details"][f"condition_{k}"]["margin_cm"]) for k in (1, 2, 3, 4)]
            return margins, dist, lift

        def _rec_step(action):
            env.step(torch.as_tensor(action, dtype=torch.float32, device=dev).reshape(1, 12))
            points, links = particles(), link_poses()
            obs.update(points, links)
            for arm in (env.left_arm, env.right_arm):
                if not bool(arm.data.joint_pos.isfinite().all()) or not bool(arm.data.joint_vel.isfinite().all()):
                    raise ValueError("nonfinite robot state in recovery branch")
            g = checker_details()
            _rec_step.last = _rec_measure(points, links, g)
            return int(g["conditions_passed"]), int(g["conditions_total"]), bool(g["success"])

        def _rec_chunk(images, joint):
            batch = make_observation(images, joint)
            batch = pre(batch) if pre else batch
            with torch.inference_mode():
            ...
            if hasattr(chunk, "detach"):
                chunk = chunk.detach().cpu().numpy()
            chunk = np.asarray(chunk)[0].astype(np.float32, copy=True)
            if chunk.shape != (50, 12) or not np.isfinite(chunk).all():
                raise RuntimeError(f"recovery chunk has shape {chunk.shape}")
            return chunk

        def _progress_summary(trace, total):
            """Distinguish sustained progress from transient crossings."""
            full = [c == total for c in trace]
            entries = sum(1 for k in range(len(full)) if full[k] and (k == 0 or not full[k - 1]))
            first_full = next((k for k, f in enumerate(full) if f), None)
            best = max(trace) if trace else 0
            regressions = sum(1 for k in range(1, len(trace)) if trace[k] < trace[k - 1])
            return {"max_conditions": int(best), "first_full_step_offset": first_full,
                    "full_entries": int(entries), "condition_regressions": int(regressions),
                    "final_policy_conditions": int(trace[-1]) if trace else None}

        search_started = time.monotonic()
        attempts, ex_images, ex_state, ex_action, ex_meta = [], [], [], [], []
        out_path = Path(args.recovery_out)

        def _flush():
            out_path.write_text(json.dumps({
                "schema": 2, "garment": args.garment, "seed": args.seed,
                "telemetry": ("v7: per policy step margin_trace [c1..c4 margin_cm], "
                              "gripper_distance_trace [left, right] m (gripper/jaw link origin to nearest "
                              "particle), lift_trace m (max particle height above the pre-policy cloth); "
                              "terminal_* after the settle"),
                "policy_path": str(args.policy_path),
                "student_execution_horizon": int(args.n_action_steps),
                "roots": {str(k): {"conditions_passed": v["conditions_passed"]}
                          for k, v in sorted(recovery_snapshots.items())},
                "candidates": [{"horizon": h, "seed": s} for h, s in args.recovery_candidates],
                "obs_every": args.recovery_obs_every, "settle_steps": args.terminal_settle_steps,
                "max_seconds": args.recovery_max_seconds,
                "student_terminal_success": bool(terminal_success),
                "attempts": attempts,
            }, indent=1) + "\n")

        for root_step in sorted(recovery_snapshots):
            snap = recovery_snapshots[root_step]
            for cand_index, (horizon, cand_seed) in enumerate(args.recovery_candidates):
                attempt = {"root": int(root_step), "candidate": cand_index, "horizon": horizon,
                           "seed": cand_seed, "root_conditions": snap["conditions_passed"]}
                if time.monotonic() - search_started > args.recovery_max_seconds:
                    attempt.update(state="not_run", reason="recovery wall-clock budget exhausted")
                    attempts.append(attempt)
                    continue
                started = time.monotonic()
                try:
                    _restore_state(snap)          # raises if cloth RMS > 1e-5 m
                    torch.manual_seed(cand_seed)
                    torch.cuda.manual_seed_all(cand_seed)
                    actions, trace, kept, plan, total = [], [], [], None, 4
                    margin_trace, gripper_trace, lift_trace = [], [], []
                    for t in range(root_step, args.steps):
                        rel = t - root_step
                        sample = rel % args.recovery_obs_every == 0
                        if rel % horizon == 0 or sample:
                            images = render_images()
                            joint = _rec_joint()
                            if sample:
                                kept.append((t, np.stack([images[k] for k in camera_keys]).astype(np.uint8),
                                             joint.copy()))
                            if rel % horizon == 0:
                                plan = _rec_chunk(images, joint)
                        a = plan[rel % horizon].copy()
                        passed, total, _ = _rec_step(a)
                        actions.append(a)
                        trace.append(passed)
                    ...
                    attempt.update(state="completed", settled_success=bool(settled),
                                   terminal_conditions=int(passed_s), conditions_total=int(total),
                    ...
                    if settled:
                        ...
                        for t, img, st in kept:
                            rel = t - root_step
                            if rel + 50 <= len(stream):
                                ex_images.append(img)
                                ex_state.append(st)
                                ex_action.append(stream[rel:rel + 50])
                                ex_meta.append((root_step, cand_index, horizon, cand_seed, t))
                except Exception as exc:          # recorded, never silently dropped
                    attempt.update(state="error", error=repr(exc)[:500])
                attempt["wall_seconds"] = round(time.monotonic() - started, 2)
                attempts.append(attempt)
                log(f"RECOVERY_ATTEMPT root={root_step} cand={cand_index} h={horizon} "
                    f"state={attempt['state']} settled={attempt.get('settled_success')}")
                _flush()
        _flush()
        meta = np.asarray(ex_meta, dtype=np.int64).reshape(-1, 5)
        np.savez_compressed(
            args.recovery_examples_out,
            images=(np.stack(ex_images) if ex_images else np.zeros((0, 3, 480, 640, 3), np.uint8)),
            state=(np.stack(ex_state) if ex_state else np.zeros((0, 12), np.float32)),
            action=(np.stack(ex_action) if ex_action else np.zeros((0, 50, 12), np.float32)),
            root=meta[:, 0], candidate=meta[:, 1], horizon=meta[:, 2], candidate_seed=meta[:, 3],
            step_index=meta[:, 4], camera_keys=np.asarray(camera_keys), garment=np.asarray(args.garment),
            episode_seed=np.asarray(args.seed))
        log(f"RECOVERY_WRITTEN attempts={len(attempts)} examples={len(ex_action)}")
    ...
    if causal_enabled:
        ...
        def _predict_observed(batch, audit_label="causal_diagnostic", predictor=None, **kwargs):
            """Call inference and prove that no simulator action elapsed."""
            sim_before = _sim_counter()
            episode_before = _episode_counter()
            rng_before = _rng_digest()
            started = time.monotonic()
            chunk = (original_predict if predictor is None else predictor)(batch, **kwargs)
            torch.cuda.synchronize()
            sim_after = _sim_counter()
            episode_after = _episode_counter()
            timing = {
        ...
        def _fresh_chunk(batch, audit_label="causal_fresh_replan", predictor=None,
                        postprocessor=None):
            with torch.inference_mode():
            ...
            post_fn = post if postprocessor is None else postprocessor
            if post_fn:
                chunk = post_fn(chunk)
            if hasattr(chunk, "detach"):
                chunk = chunk.detach().cpu().numpy()
        ...
        def _run_fresh(snapshot, horizon, branch_pre=pre, branch_predictor=None,
                       branch_post=post):
            fidelity = _restore_and_validate(snapshot)
            boundary = int(snapshot["boundary"])
            trace, actions, chunks = [], [], []
            ...
            for local in range(args.causal_branch_steps):
                ...
                if local % horizon == 0:
                    ...
                    current_chunk = _fresh_chunk(
                        policy_batch_for(branch_pre, images, joint),
                        predictor=branch_predictor, postprocessor=branch_post)
                    chunks.append(current_chunk.copy())
                action = current_chunk[local % horizon].copy()
                actions.append(action)
                row = _branch_step(action)
        ...
        def _run_boundary_checkpoint_eval():
            """Compare every retained trained plan on restored snapshots only."""
            trained_paths = [Path(path) for path in boundary_eval_paths]
            if not trained_paths or any(not path.is_dir() for path in trained_paths):
                missing = [str(path) for path in trained_paths if not path.is_dir()]
                raise RuntimeError(f"trained boundary checkpoints are missing: {missing}")
            labels = [path.name for path in trained_paths]
            if len(set(labels)) != len(labels) or "baseline" in labels:
                raise RuntimeError(f"trained checkpoint directory names must be unique: {labels}")
            checkpoints = {
            ...
            def _evaluate_item(label, item, boundary, snapshot, target):
                fidelity = _restore_and_validate(snapshot)
                model = item["model"]
                model.reset()
                _set_policy_rng(causal_initial_torch_rng, causal_initial_cuda_rng)
            ...
            for boundary, snapshot in sorted(causal_snapshots.items()):
                ...
                model_records = {
                    "baseline": _evaluate_item("baseline", checkpoints["baseline"],
                                                boundary, snapshot, target)
                }
                baseline_rng = model_records["baseline"]["prediction_timing"]["rng_before_digest"]
                for path, label in zip(trained_paths, labels):
                    # save_pretrained() emits a model-shaped config without
                    # this checkout's top-level policy-choice `type`. Symlink
                    # every trained file and replace only config.json in a
                    # temporary compatibility directory; the artifact stays
                    # unchanged.
                    ...
                    item = {"path": str(path), "model": trained_model,
                            "pre": trained_pre, "post": trained_post}
                    model_records[label] = _evaluate_item(
                        label, item, boundary, snapshot, target)
                    if model_records[label]["prediction_timing"]["rng_before_digest"] != baseline_rng:
                        raise RuntimeError(f"boundary {boundary} {label} and baseline RNG streams differ")
                    del trained_model
                ...
                records[str(boundary)] = {
                    ...
                    "baseline_and_trained_same_rng": all(
                        row["prediction_timing"]["rng_before_digest"] == baseline_rng
                        for row in model_records.values()),
                    "models": model_records,
                }
            checkpoint_manifest = {
                "baseline": {"path": args.policy_path,
                              "files": _checkpoint_hashes(args.policy_path)}
            }
            checkpoint_manifest.update({
                label: {"path": str(path), "files": _checkpoint_hashes(path)}
                for path, label in zip(trained_paths, labels)
            })
            return {
                "schema_version": 2,
                "mode": "snapshot_only_boundary_replay_checkpoint_panel",
                "closed_loop_execution": False,
                "rtc_used": False,
                "queue_or_retained_prefix_used": False,
                "observation_substitution_used": False,
                "checkpoint_order": ["baseline"] + labels,
                "checkpoints": checkpoint_manifest,
                "boundaries": records,
                "all_snapshot_restores_within_tolerance": all(
                    model["snapshot_restore"]["cloth_rms_m"] <= 1e-5 and
                    model["snapshot_restore"]["joint_rms_rad"] <= 1e-5
                    for row in records.values() for model in row["models"].values()),
                "all_predictions_zero_sim_steps": all(
                    not model["simulator_stepped_during_prediction"]
                    for row in records.values() for model in row["models"].values()),
                "all_rng_restores_exact": all(
                    model["rng_restore_exact"]
                    for row in records.values() for model in row["models"].values()),
            }

        def _run_selected_boundary_execution():
            """Run ordinary fresh H10 branches from the two exact snapshots."""
            trained_path = Path(args.boundary_execute_trained_path)
            if not trained_path.is_dir():
                raise RuntimeError(f"selected execution checkpoint is missing: {trained_path}")
            compat_dir = Path(tempfile.mkdtemp(prefix="boundary-execute-"))
            ...
            trained_predictor = trained_model._get_action_chunk
            branches = {}
            try:
                for boundary, snapshot in sorted(causal_snapshots.items()):
                    if boundary not in (5, 10):
                    ...
                    branch = _run_fresh(
                        ...
                        branch_predictor=trained_predictor, branch_post=trained_post)
                    branch["checkpoint"] = str(trained_path)
                    branch["horizon"] = 10
                    branch["ordinary_fresh_h10"] = True
                    branch["cached_h50_actions_used_in_branch"] = False
                    branches[str(boundary)] = branch
            finally:
                del trained_model
                torch.cuda.empty_cache()
                shutil.rmtree(compat_dir, ignore_errors=True)
            if set(branches) != {"5", "10"}:
                raise RuntimeError(f"boundary execution expected both snapshots, got {sorted(branches)}")
            return {
                "schema_version": 1,
                "mode": "ordinary_fresh_h10_from_exact_pose3_snapshots",
                "checkpoint": str(trained_path),
                "initial_snapshot_source": "baseline cached successful H50 replay",
                "cached_h50_actions_used_in_branch": False,
                "rtc_used": False,
                "retained_prefix_queue_used": False,
                "stale_or_hybrid_observations_used": False,
                "boundaries": branches,
                "both_boundaries_settled_4_of_4": all(
                    branches[str(boundary)]["settled"]["settled_4_of_4"]
        ...
        elif args.boundary_execute_trained_path:
            boundary_eval_result = None
            boundary_execution_result = _run_selected_boundary_execution()
            branches["boundary_execution"] = boundary_execution_result
            with open(args.boundary_execute_out, "w") as boundary_execute_file:
