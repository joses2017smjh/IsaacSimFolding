...
class Driver:
    ...
    def baseline_repeat(self) -> dict | None:
        """v4 spends no new baseline run. The pooled baseline is preregistered
        from the two matched runs already measured on identical rows, seeds
        and runner code paths (v2 campaign and v3 campaign); their driver
        states are read verbatim rather than retyped."""
        ...
        v3 = json.loads((self.root.parent / "20260923-recovery-supervision-v3/ledger/driver_state.json").read_text())
        runs = [dict(v2["baseline_dev"]["h10"], source="v2-campaign run"),
                dict(v3["baseline"]["h10_runs"][1], source="v3-campaign run")]
        assert [r["settled"] for r in runs] == [2, 0], "recorded baseline runs changed on disk"
        self.state["baseline"] = {"h10_runs": runs,
                                  "h50": dict(v2["baseline_dev"]["h50"], source="v2-campaign run")}
        self.event("pooled preregistered baseline loaded", h10="2/16", h50=runs and 4)
        return self.state["baseline"]
    ...
    def attempt_plan(self, k: int) -> dict | None:
        if k == 1:
            return {"attempt": 1, "dataset": "recovery",
                    "init_checkpoint": self.manifest["baseline_checkpoint"]["path"],
                    "checkpoint_rule": self.manifest["training"].get("checkpoint_rule", "default"),
