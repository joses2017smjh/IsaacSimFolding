...
def verify_physx_remap(authored, rest, to_weld, to_orig, live_count):
    ...
    # PhysX omits both remap attributes for an unwelded mesh. Require
    # identical authored/rest topology and no duplicate positions before
    # accepting its identity mapping; a missing map on welded cloth fails.
    absent = lambda value: value.size == 0 or (value.shape == () and value.item() is None)
    if absent(to_weld) and absent(to_orig):
        _, unique = weld_correspondence(authored)
        if live_count != len(authored) or len(unique) != len(authored):
            raise ValueError("Absent PhysX remaps on a welded or mismatched mesh")
        proof = verify_cooked_rest(rest, authored, live_count)
        proof["proof_source"] = "Unwelded PhysX topology: both maps absent, unique source/rest positions and live count identical"
        return np.arange(live_count, dtype=np.int64), proof
...
def bind_landmarks(obj, mesh_path):
    ...
    if live.ndim != 2 or live.shape[1] != 3 or not np.isfinite(live).all():
