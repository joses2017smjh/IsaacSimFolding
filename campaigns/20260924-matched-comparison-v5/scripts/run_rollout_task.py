...
v5 evaluates exactly two pinned checkpoints -- the immutable baseline and the
v4 candidate a2-step000250 -- on the same predeclared rows.  The simulator
and controller remain the validated horizon-pilot runner (v4's byte-identical
snapshot); this adapter supplies a row, a checkpoint and an output location.
It neither changes inference semantics nor evaluates anything unpinned.
