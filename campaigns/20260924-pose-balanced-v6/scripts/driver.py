"""Restartable, duplicate-safe driver for v6: pose-balanced supervision.
...
resubmitted while its key holds a live or finished job):
    ...
    search.collect  targeted recovery search: 16 training-only rows at the
                    P_B and P_A poses, baseline as student (v4's machinery)   16 tasks
    search.compile  per-pose coverage gate on the new search (exit 4 = the
                    supervision is still missing: stop, train nothing), then
                    v4's pinned corpus + new examples, pose-balanced weights   CPU
...
class Driver:
    ...
    def search(self) -> dict | None:
        ...
        self.event(f"per-pose coverage gate passed={gate['passed']}",
                   per_pose={p: g.get("successful_branches") for p, g in gate.get("per_pose", {}).items()},
    ...
    def advance(self) -> str:
        ...
        if not gate.get("passed"):
            return self.finish(f"per-pose coverage gate unmet: {gate.get('unmet_requirements')}; "
                               "the supervision is still missing, so nothing is trained")
    ...
    def describe(self):
        ...
        if g:
            ...
            parts.append(f"targeted search: {per or '-'} settled (gate {'pass' if g.get('passed') else 'FAIL'})")
        ...
        return active, "; ".join(parts) or "targeted search pending", blocker, nxt
