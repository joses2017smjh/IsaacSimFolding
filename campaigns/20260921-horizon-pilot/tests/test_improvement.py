...
from lehome_fold.folding_geometry import (authored_mesh, weld_correspondence,
    validate_indices, verify_initial_correspondence, verify_cooked_rest, verify_physx_remap, bind_landmarks, fresh_geometry, mapped_positions_cm)
...
class ImprovementTests(unittest.TestCase):
    ...
    def test_physx_remap_accepts_solver_reordering_rejects_corruption(self):
        authored = np.array([[0., 0, 0], [1, 0, 0], [0, 0, 0], [2, 0, 0]])
        forward, reverse = np.array([2, 0, 2, 1]), np.array([1, 3, 0])
        mapping, proof = verify_physx_remap(authored, authored, forward, reverse, 3)
        np.testing.assert_array_equal(mapping, forward)
        for f, r in [(forward, reverse[::-1]), (forward + 1, reverse), (forward, reverse[:2])]:
            with self.assertRaises(ValueError):
                verify_physx_remap(authored, authored, f, r, 3)
        with self.assertRaises(ValueError):
            verify_physx_remap(authored, authored + .003, forward, reverse, 3)

    def test_unwelded_missing_maps_require_exact_unique_topology(self):
        authored = np.arange(18, dtype=float).reshape(6, 3)
        mapping, proof = verify_physx_remap(authored, authored, None, None, 6)
        np.testing.assert_array_equal(mapping, np.arange(6))
        for rest, count, forward, reverse in [(authored, 5, None, None),
                (authored + .003, 6, None, None), (authored, 6, None, np.arange(6))]:
            with self.assertRaises(ValueError):
                verify_physx_remap(authored, rest, forward, reverse, count)
        duplicate = np.vstack([authored, authored[0]])
        with self.assertRaises(ValueError):
            verify_physx_remap(duplicate, duplicate, None, None, len(duplicate))

    def test_full_binding_handles_actual_unwelded_and_welded_assets(self):
        from unittest.mock import patch
        from pxr import Usd
        # Exercise bind_landmarks itself, including its post-validation guard.
        # Stand in for GPU USD schema access; use actual garment USD points.
        for garment in ("Pant_Long_Seen_0", "Top_Short_Seen_0", "Top_Short_Seen_3"):
            row = next(r for r in self.inventory["garments"] if r["garment_id"] == garment)
            authored, _ = authored_mesh(row["mesh_path"])
            mapping, unique = weld_correspondence(authored)
            _, first = np.unique(mapping, return_index=True)
            welded = len(unique) != len(authored)
            attrs = {"physxParticle:weldedVerticesRemapToWeld": mapping if welded else None,
                     "physxParticle:weldedVerticesRemapToOrig": first if welded else None}
            prim = SimpleNamespace(GetAttribute=lambda name: SimpleNamespace(Get=lambda: attrs[name]))
            obj = SimpleNamespace(_prim=prim, check_points=row["authored_landmarks"],
                _cloth_prim_view=SimpleNamespace(get_world_positions=lambda: torch.tensor(unique).unsqueeze(0)))
            schema = SimpleNamespace(PhysxParticleClothAPI=lambda prim:
                SimpleNamespace(GetRestPointsAttr=lambda: SimpleNamespace(Get=lambda: authored)))
            with patch.dict(sys.modules, {"pxr.PhysxSchema": schema}):
                proof = bind_landmarks(obj, row["mesh_path"])
            self.assertTrue(proof["runtime_correspondence_verified"])
            self.assertEqual(proof["particle_count"], len(unique))
            self.assertTrue(np.isfinite(mapped_positions_cm(obj, obj.check_points)).all())
