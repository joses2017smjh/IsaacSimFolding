...
class RolloutAuditTests(unittest.TestCase):
    def test_active_controller_inference_blocks_before_env_step(self):
        source = (RENDER / "policy_rollout51.py").read_text()
        prediction = source.index("with torch.inference_mode():\n                act = policy.select_action(batch)")
        step = source.index("env.step(torch.from_numpy(a.astype(np.float32))", prediction)
        self.assertLess(prediction, step)
        self.assertIn('"sim_step_delta": sim_after - sim_before', source)
        self.assertIn('"inference_blocks_before_env_step": True', source)
