...
def main() -> int:
    ...
    if args.phase in ("recovery", "recovery_smoke"):
        ...
        # v6: a row may carry its own roots (P_B rows search early roots, where
        # v4 found nearly all P_B successes); otherwise the config's roots.
        roots = row.get("recovery_roots") or cfg["roots"]
        command.extend([
            "--recovery_out", str(dest / "recovery.json"),
            "--recovery_examples_out", str(dest / "recovery_examples.npz"),
            "--recovery_roots", ",".join(str(r) for r in roots),
