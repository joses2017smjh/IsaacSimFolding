...
def load_raster_subset(pattern: str, files_requested: int, frames_per_file: int,
                       *, heldout: set[Path], mode: str = "episode_start"
                       ) -> tuple[np.ndarray, np.ndarray, np.ndarray, list[dict]]:
    """Anchor frames from demonstration episodes.

    mode "episode_start" (every prior campaign): the FIRST frames_per_file
    frames of each file -- the home-to-ready snap only. v3 attempt 2 showed
    this anchor cannot protect whole-episode behaviour: both training losses
    fell while whole-episode retention breached at every checkpoint, because
    nothing in the batch ever showed the model a grasp or a fold.

    mode "whole_episode": frames_per_file frames spread evenly across each
    ENTIRE episode -- approach, grasp, fold, release -- the same construction
    as the retention guard, on anchor files the guard never sees.
    """
    ...
    if mode not in ("episode_start", "whole_episode"):
    ...
    for path in selected:
        ...
        if mode == "whole_episode":
            idx = np.unique(np.linspace(0, len(x) - 1, frames_per_file).round().astype(np.int64))
        else:
            idx = np.arange(frames_per_file)
        images.append(x[idx])
        state.append(s[idx])
        action.append(a[idx])
        records.append({"path": str(path), "sha256": digest(path), "mode": mode,
                        "frames_used": idx.tolist(), "garment": garment, "episode": episode})
...
def main() -> int:
    ...
    ap.add_argument("--anchor-mode", choices=("episode_start", "whole_episode"),
                    default="episode_start",
                    help="how anchor frames are drawn from each demonstration episode")
    ...
    anchor_x, anchor_s, anchor_a, anchor_records = load_raster_subset(
        args.anchor_glob, args.anchor_files, args.anchor_frames,
        heldout=set(heldout_paths), mode=args.anchor_mode)
    ...
    summary = {
        ...
        "anchor": {"sampling_fraction": n_anchor / args.batch_size, "mode": args.anchor_mode,
